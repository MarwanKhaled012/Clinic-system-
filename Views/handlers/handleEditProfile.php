...
// Patient
    ...
    if ($resulteditDData){
        echo " <script>alert(' تم تحديث البيانات بنجاح');</script> ";
        header('Refresh:1 ; URL = ../doctors.php');
        die();
    }
}
    ...
    $updatePatientHistory = " UPDATE labanalysis SET chestPainType='$pCPT',bloodPressure='$pBP',cholesterol='$pC',maxHeartRate='$pMHR',exerciseAngina='$pEA',plasmaGlucose='$pPG',skinThickness='$pST',insulin='$pI',bmi='$pBMI',diabetesDegree='$pDD',hypertension='$pHT',heartDisease='$pHD',Pregnancies='$pPreg' WHERE patientID = '$pID'";
    // if (($_FILES['pRay']['name'] == '')){
    // } else {
    //     $pRay = $_FILES['pRay'];
    //     $photo_name = $pRay['name'];   
    //     $photo_tmp_name = $pRay['tmp_name'];
    //     $photo_extension = pathinfo($photo_name, PATHINFO_EXTENSION);
    //     $photo_new_name = uniqid() . "." . $photo_extension;
    //     move_uploaded_file($photo_tmp_name, "../../Uploads/Images/$photo_new_name");
    //     $updatePatientHistory = " UPDATE labanalysis SET chestPainType='$pCPT',bloodPressure='$pBP',cholesterol='$pC',maxHeartRate='$pMHR',exerciseAngina='$pEA',plasmaGlucose='$pPG',skinThickness='$pST',insulin='$pI',bmi='$pBMI',diabetesDegree='$pDD',hypertension='$pHT',heartDisease='$pHD',Pregnancies='$pPreg', ray = '$photo_new_name' WHERE patientID = '$pID'";
    //     }
    ...
    echo " <script>alert(' تم تحديث البيانات بنجاح');</script> ";
    header('Refresh:1 ; URL = ../receptionistReservations.php');
    $conn->close();
    die();
}
    ...
    if (($_FILES['prescription']['name'] == '')){
        ...
        header('Refresh:1 ; URL = ../nurseReservations.php');
        die();
    } else {
